...
unsigned long remote_signal=0;
//order of singals: left, right, forward, back
unsigned long IR_controls[5]={551492580,551525220,551512980,551545620,551508135};
unsigned long speedcontrol=0;
int speed=255;
...
void change_speed(unsigned long remote_signal){
  if(remote_signal==IR_controls[4]){
    speed+=51;
    if(speed>=255)
      speed=51;
...
void move(unsigned long remote_signal){
  if(remote_signal==IR_controls[0]){
     analogWrite(leftSpeed,speed); // enable on
      digitalWrite(leftDirA,HIGH); //one way
      digitalWrite(leftDirB,LOW);
      analogWrite(rightSpeed,speed); // enable on
      digitalWrite(rightDirA,HIGH); //one way
      digitalWrite(rightDirB,LOW);
  }

  else if(remote_signal==IR_controls[1]){
     analogWrite(leftSpeed,speed); // enable on
  ...
  else if(remote_signal==IR_controls[2]){
     analogWrite(leftSpeed,speed); // enable on
      digitalWrite(leftDirA,LOW); //one way
      digitalWrite(leftDirB,HIGH);
      analogWrite(rightSpeed,speed); // enable on
  ...
  else if(remote_signal==IR_controls[3]){
     analogWrite(leftSpeed,speed); // enable on
      digitalWrite(leftDirA,HIGH); //one way
      digitalWrite(leftDirB,LOW);
      analogWrite(rightSpeed,speed); // enable off
      digitalWrite(rightDirA,LOW); //one way
      digitalWrite(rightDirB,HIGH);
  }
